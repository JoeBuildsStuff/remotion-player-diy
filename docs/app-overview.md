...
- A transport bar for split/delete, playback controls, loop, and timeline zoom.
- A bottom timeline with tracks, a ruler, selected clips, trim handles, and a playhead.
...
All clips share timing and layout fields such as `startFrame`, `durationInFrames`, `trackIndex`, `x`, `y`, `width`, `height`, `rotation`, `opacity`, and fade settings. Media clips also carry source and upload state. Text clips add typography, stroke, and background fields.
