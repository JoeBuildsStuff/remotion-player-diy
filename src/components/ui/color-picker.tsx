...
export const ColorPicker = ({
  ...
  const [{ hue: initialHue, saturation: initialSaturation, lightness: initialLightness, alpha: initialAlpha }] = useState(() => {
    const selectedColor = parseColorOrFallback(value, defaultValue)
    const h = selectedColor.hue()
    return {
      hue: Number.isFinite(h) ? h : 0,
      saturation: selectedColor.saturationl(),
      lightness: selectedColor.lightness(),
      alpha: selectedColor.alpha() * 100,
    }
  })
  ...
  // Keep onChange in a ref so its identity doesn't retrigger the effect.
  const onChangeRef = useRef(onChange)
  useEffect(() => {
    onChangeRef.current = onChange
  })

  // Notify parent of changes — skip the initial mount so we don't overwrite
  // the parent's value with our defaulted HSL on open.
